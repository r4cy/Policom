...
func waitIdle(t *testing.T, token string) {
	for {
...
func login(t *testing.T) string {
	var token LoginReply
	data := bytes.NewBufferString(`{"username":"admin", "password":"password"}`)
...
func prepare(t *testing.T) {
	...
	updateStatus, err := status(login(t))
...
// this must not contain t because it runs in a waited goroutine
func status(token string) (string, error) {
	req, err := http.NewRequest(http.MethodGet, address+"/api/db/status", nil)
	if err != nil {
		return "", err
	}
	req.Header.Add("Authorization", "Bearer "+token)
...
func stats(t *testing.T) UpdateStats {
	req, err := http.NewRequest(http.MethodGet, address+"/api/db/stats", nil)
	require.NoError(t, err, "cannot make request")
	token := login(t)
	req.Header.Add("Authorization", "Bearer "+token)
	resp, err := client.Do(req)
